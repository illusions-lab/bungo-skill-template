...
2. `generate-assets.yml` — OG / SNS 画像を生成して OG を自動コミット
3. `deploy-pages.yml` — 同梱の `generate/web/` を Vite ビルド → GitHub Pages 配信
...
| GitHub Pages が更新されない | OG 未生成 / build 対象ズレ | `node generate/bin/skill-generator.js generate --only=og` 後に `cd generate/web && npm run build` で再現確認 |
