...
- `repository-open-graph.png`（GitHub OG 画像、PNG）
...
CI ではこれを自動化（`deploy-pages.yml` が WEB_SOURCE Variable から fetch）。
...
1. `validate-skill.yml` — YAML スキーマ検証 + OG smoke test
2. `generate-assets.yml` — OG / SNS 画像を生成して自動コミット
3. `deploy-pages.yml` — `generate-assets` 完了後、web source を upstream から clone → Vite ビルド → GitHub Pages 配信
   - **要設定**: 各 skill repo の Settings → Variables → New variable に `WEB_SOURCE=illusions-lab/skill-web@main` を追加
   - 未設定時は Pages デプロイは skip される（その他のワークフローには影響なし）
...
| GitHub Pages が更新されない | `workflow_run` の race | `deploy-pages.yml` を `workflow_dispatch` で手動実行 |
